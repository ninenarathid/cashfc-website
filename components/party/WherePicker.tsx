...
export default function WherePicker(
  ...
  return (
    <div className="flex flex-col gap-2 rounded-lg border border-line bg-bg/40 p-2.5">
      ...
      {value?.map ? (
        <div className="flex flex-wrap items-center gap-2">
          <span className="rounded-lg border border-accent/50 bg-accent/10 px-3 py-1.5 text-lead text-accent">
            {spotText(value)}
          </span>
        ...
        <>
          ...
          {hits.map((m) => (
            <button key={m.name} type="button"
              ...
              <span className="text-lead text-ink">{m.name}</span>
              {m.region && (
                <span className="font-data text-ui uppercase tracking-[0.1em] text-muted">
                  {m.region}
                </span>
              )}
              {m.sub && <span className="text-read text-muted">{m.sub}</span>}
            </button>
          ))}
...
export function SpotChip({ spot }: { spot: Spot | undefined }) {
  ...
  if (!text) return null;
  return (
    <span className="rounded-full border border-line px-2 py-[2px] font-data text-ui text-muted">
      📍 {text}
    </span>
  );
}
