      ...
      <button type="button" onClick={() => setOpen(true)}
        ...
        <span className="relative z-[1] flex w-full items-end justify-between gap-3 p-3">
          <span className="flex min-w-0 flex-col">
            ...
            {chosen && (
              <span className="font-data text-meta uppercase tracking-[0.14em] text-ink/70">
          ...
          <span className="shrink-0 rounded-lg border border-line/70 bg-bg/80 px-2.5 py-1 text-read text-ink/90">
          ...
          return (
            <button key={k} type="button"
                    ...
                    style={on ? { borderColor: KIND_COLOR[k], color: KIND_COLOR[k],
                              ...
                              : undefined}
                    className={`flex items-center gap-2 rounded-full border px-3.5 py-1.5 text-title transition-colors ${
          ...
          {groups.map((g) => {
            ...
            return (
              <button key={g} type="button" onClick={() => setGroup(g)}
                      className={`rounded-full border px-3 py-1 text-lead transition-colors ${
        ...
        {rows.map((c) => {
          ...
          return (
            <button key={c.key} type="button" disabled={shut}
              ...
              {c.badge && (
                <span className="absolute left-2 top-2 z-[1] rounded-md border border-line/70 bg-bg/80 px-2 py-[2px] font-data text-read font-bold text-ink/90">
                  {c.badge}
                </span>
              )}
              ...
              {shut && (
                <span className="absolute inset-x-2 top-1/2 z-[1] -translate-y-1/2 rounded-md bg-bg/85 px-2 py-1 text-center text-read text-ink">
                  {t("pf.wontFit")}
                </span>
              )}

              <span className="relative z-[1] flex w-full flex-col gap-0.5 p-2.5">
                <span className="truncate font-display text-head font-semibold text-ink">
                  {c.name}
                </span>
                ...
                <span className="flex items-baseline justify-between gap-1.5">
                  <span className="truncate text-read text-ink/70">
                    {c.duty && c.duty !== c.name ? c.duty
                      : c.short && c.short !== c.badge ? c.short : " "}
                  </span>
                  <span className="shrink-0 font-data text-ui uppercase tracking-[0.1em] text-ink/70">
                    {SHAPE_SIZE[c.shape] || "—"}
                  </span>
                </span>
              </span>
