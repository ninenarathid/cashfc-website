        ...
        <BlockRow key={b.id} block={b} index={i} last={i === body.length - 1}
          ...
          {b.kind === "text" ? (
            ...
            <textarea
              ...
              className="w-full resize-none border-0 bg-transparent px-1 py-0.5 text-[13.5px] leading-relaxed text-ink outline-none focus:bg-surface/40 focus-visible:outline-none placeholder:text-muted" />
          ) : (
            <div className="flex flex-col gap-1.5 py-1">
              {b.url ? (
                ...
                <span className="skeleton flex h-32 w-56 items-center justify-center rounded-lg text-[11.5px] text-muted">
                  {t("pf.uploading", { n: 1 })}
                </span>
              )}
              <input value={b.caption ?? ""}
                     ...
                     className="w-full max-w-md border-0 bg-transparent px-1 text-[12px] italic text-muted outline-none focus:bg-surface/40 focus-visible:outline-none placeholder:text-muted/60" />
            </div>
          )}
        </BlockRow>
      ))}
    ...
    <div {...handlers}
      ...
      <div className="absolute right-0 top-0 flex shrink-0 gap-1 opacity-0 transition-opacity focus-within:opacity-100 group-hover/blk:opacity-100 max-sm:opacity-60">
        <button type="button" onClick={() => onMove(-1)} disabled={index === 0}
                ...
                className="rounded border border-line bg-surface px-1.5 text-[11px] text-muted hover:text-ink disabled:opacity-30">
          ↑
        </button>
        ...
        <button type="button" onClick={onDrop} disabled={pending}
                ...
                className="rounded border border-chili/50 bg-surface px-1.5 text-[11px] text-chili hover:bg-chili/10 disabled:opacity-30">
          ✕
        </button>
      </div>
    </div>
