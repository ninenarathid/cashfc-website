      ...
      {body.map((b) => {
        if (b.kind === "text") {
          ...
          return (
            <div key={b.id} className="flex flex-col gap-2">
              ...
              <p className="whitespace-pre-wrap break-words text-head leading-relaxed text-ink/90">
                <Linkify text={b.text} />
              </p>
              {seen.map((v) => (
                <div key={v.id}
        ...
        return (
          <figure key={b.id} className="flex flex-col gap-1">
            ...
            {b.caption && (
              <figcaption className="text-title text-muted">{b.caption}</figcaption>
            )}
          </figure>
        );
      })}
  ...
  return (
    <div {...handlers}
      ...
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        {err && <span className="text-lead text-chili">{err}</span>}
      </div>
        ...
        <BlockRow key={b.id} block={b} index={i} last={i === body.length - 1}
          ...
          {b.kind === "text" ? (
            ...
            <textarea
              ...
              className="w-full resize-none border-0 bg-transparent px-1 py-0.5 text-head leading-relaxed text-ink outline-none focus:bg-surface/40 focus-visible:outline-none placeholder:text-muted" />
          ) : (
            <div className="flex flex-col gap-1.5 py-1">
              {b.url ? (
                ...
                <span className="skeleton flex h-32 w-56 items-center justify-center rounded-lg text-lead text-muted">
                  {t("pf.uploading", { n: 1 })}
                </span>
              )}
              <input value={b.caption ?? ""}
                     ...
                     className="w-full max-w-md border-0 bg-transparent px-1 text-title italic text-muted outline-none focus:bg-surface/40 focus-visible:outline-none placeholder:text-muted/60" />
            </div>
          )}
        </BlockRow>
      ))}
      ...
      <div className="flex flex-wrap items-center gap-2 pt-1">
        <button type="button"
                ...
                className="rounded-lg border border-line px-3 py-1 text-title text-muted hover:border-accent/60 hover:text-ink">
          {t("pf.paragraph")}
        </button>
        <span className="text-lead text-muted">
          {t("pf.dropAnywhere")}
    ...
    <div {...handlers}
      ...
      <div className="absolute right-0 top-0 flex shrink-0 gap-1 opacity-0 transition-opacity focus-within:opacity-100 group-hover/blk:opacity-100 max-sm:opacity-60">
        <button type="button" onClick={() => onMove(-1)} disabled={index === 0}
                ...
                className="rounded border border-line bg-surface px-1.5 text-lead text-muted hover:text-ink disabled:opacity-30">
          ↑
        </button>
        ...
        <button type="button" onClick={onDrop} disabled={pending}
                ...
                className="rounded border border-chili/50 bg-surface px-1.5 text-lead text-chili hover:bg-chili/10 disabled:opacity-30">
          ✕
        </button>
      </div>
    </div>
