        ...
        {ROULETTES.map((r) => {
          ...
          return (
            <button key={r.id} type="button" onClick={() => toggle(r.name)}
                    title={r.full}
                    className={`flex items-center gap-1.5 rounded-full border px-3 py-[3px] text-lead transition-colors ${
                      on ? "border-accent bg-accent/15 text-accent"
                         : "border-line text-muted hover:border-muted hover:text-ink"}`}>
              <span className={`grid size-4 shrink-0 place-items-center rounded-[3px] border text-meta leading-none ${
