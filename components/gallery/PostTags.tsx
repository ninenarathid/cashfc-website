...
export default function PostTags(
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {waitingOnMe && (
        <p className="rounded-lg border border-accent/40 bg-accent/10 px-3 py-2 text-ui leading-relaxed text-ink/85">
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        {[...people].map(([key, who]) => {
          ...
          return (
            <span key={key}
                  className={`flex items-center gap-1.5 rounded-full border py-0.5 pl-2.5 pr-1.5 text-ui ${
              ...
              {who.pins > 0 && (
                <span title={t("gallery.tagPinned")} className="text-label">📍</span>
              )}
              {guest && (
                <span className="text-label italic text-muted">
                  · {t("gallery.tagGuest")}
                </span>
              )}
              {who.pending && (
                <span className="text-label italic">· {t("gallery.tagPending")}</span>
              )}
              {canConfirm && (
                <button onClick={() => confirm(who)} disabled={busy}
                        title={isMe ? t("gallery.tagConfirm") : t("gallery.tagConfirmFor")}
                        className="rounded-full border border-jade/50 px-2 text-meta text-jade hover:bg-jade/10 disabled:opacity-40">
                  ✓
                </button>
              )}
              {canRemove && (
                <button onClick={() => remove(who)} disabled={busy}
                        ...
                        className="rounded-full border border-line px-1.5 text-meta text-muted hover:border-chili hover:text-chili disabled:opacity-40">
                  ✕
                </button>
              )}
            </span>
        ...
        {!people.size && (
          <span className="text-ui text-muted">{t("gallery.tagNone")}</span>
        )}
        ...
        {pinned > 0 && (
          <button onClick={() => onReveal(!revealAll)}
                  className={`rounded-full border px-2.5 py-0.5 text-ui transition-colors ${
