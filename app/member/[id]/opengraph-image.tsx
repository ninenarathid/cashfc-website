  ...
  return new ImageResponse(
    (
      <div style={{
        ...
        justifyContent: "space-between", background: "#17130e",
        color: "#efe6d3", padding: 64,
      }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div style={{ color: "#e8a33d", letterSpacing: 8, fontSize: 26 }}>
            CAFE AND SHABU · TONBERRY [ELEMENTAL]
          </div>
          <div style={{
            width: 22, height: 22, borderRadius: 999, background: "#e8a33d",
          }} />
        </div>
        <div style={{ display: "flex", flexDirection: "column" }}>
          <div style={{ fontSize: 78, fontWeight: 700 }}>{m?.name ?? "FC Member"}</div>
          <div style={{ color: "#9c8f78", fontSize: 30, marginTop: 6 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
          <div style={{ display: "flex", gap: 14 }}>
            {(m?.tags ?? []).slice(0, 4).map((t) => (
              <div key={t} style={{
                border: "2px solid #3a3226", borderRadius: 999,
                padding: "8px 26px", fontSize: 26, color: "#9c8f78",
              }}>
                {t.toUpperCase()}
              </div>
            ))}
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end" }}>
            <div style={{ fontSize: 24, color: "#9c8f78", letterSpacing: 6 }}>BEST PARSE</div>
